...
//  MovieQuizTests
//
//  Created by Vladislav Vintenbakh on 12/11/23.
//
...
struct StubNetworkClient: NetworkRouting {
    ...
    let emulateError: Bool // this parameter is needed for the stub to emulate either a network error or success response
...
class MoviesLoaderTests: XCTestCase {
    func testSuccessLoading() throws {
        ...
        // create an expectation as loadMovies is an async function
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies):
                // compare the data with what was expected
                XCTAssertEqual(movies.items.count, 2)
                expectation.fulfill()
            case .failure(_):
                // we don't expect an error and have to fail the test if we get it
